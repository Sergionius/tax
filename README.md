...
`tax` connects your iPhone to open Orca workspaces and terminals on your Mac through a relay you control. The app renders ANSI/TUI output, sends input to the selected PTY, creates and closes terminals, runs console agents (Pi, Oh My Pi, Claude Code, Codex) and any CLI program, and lets you browse and edit workspace files safely.
...
- Send terminal-aware completion notifications from Pi, OMP, Claude Code, Codex, or any command wrapped with `tax run`.
...
The relay receives terminal input/output and file contents only as ciphertext. Separately, notification context/logs can contain command output and are sent to the backend in plaintext over HTTPS. Ambiguously delivered terminal input is never replayed automatically. See [`docs/PRIVACY.md`](docs/PRIVACY.md) for what is encrypted, what the backend and Apple can see, and what is stored.

## Requirements

- Orca running on a Mac. The Mac must stay powered on and awake (for example with `caffeinate` or Energy settings) whenever you want to connect; the TAX remote host and Orca runtime live there, while the relay runs on your server.
- Python 3.11+ and pipx for the CLI; Node.js 24+ for the terminal bridge and extension tests. Xcode is required to build the iPhone app.
...
### 2. Install and configure the CLI on the Mac

Clone the repository and install the CLI (install Python 3.11+, pipx and Node.js 24+ first):

```bash
git clone https://github.com/Sergionius/tax.git
cd tax
./scripts/install.sh
```
...
### Pi, OMP, Claude Code, and Codex

TAX can send the same terminal-aware completion notifications for Pi, OMP, Claude Code, and Codex. Pi uses `extensions/tax-push.ts`; OMP uses `extensions/tax-push-omp.ts`. Enabling the integrations below does not replace either extension.
...
#### Pi and Oh My Pi (OMP) environment

Both extensions read `TAX_SERVER` and `TAX_API_KEY` from the agent process environment, **not** from `tax config`. After configuring the CLI, load those values into the shell that starts Pi or OMP without printing secrets:

```bash
eval "$(python3 - <<'PY'
import json, pathlib, shlex
config = json.loads((pathlib.Path.home() / '.config/tax/config.json').read_text())
for variable, key in [('TAX_SERVER', 'server'), ('TAX_API_KEY', 'api_key')]:
    value = config.get(key)
    if not value:
        raise SystemExit('Configure the CLI server and API key first')
    print(f'export {variable}={shlex.quote(value)}')
PY
)"
```

Start the agent in that same Orca terminal after loading the environment. Existing processes do not inherit later environment changes. Do not print or commit these values. Claude Code and Codex hooks read the CLI configuration directly.

#### Oh My Pi (OMP)

`./scripts/install.sh` links the package automatically when `omp` is installed. To link manually:

```bash
omp plugin link /absolute/path/to/tax --scope user
```

The package manifest loads only `extensions/tax-push-omp.ts` in OMP; the existing Pi extension remains registered separately. For a one-off check without installing the package:

```bash
omp --extension /absolute/path/to/tax -p "Reply with done"
```

OMP sends one notification from its terminal `session_stop` event, after automatic retries and agent-owned background jobs have settled. The payload uses `source=omp-extension`, `agent=omp`, and the same TAX/Orca environment variables as the Pi extension.
...
Install the pinned development tool first: `python3 -m pip install uv==0.12.13` in a dedicated tool environment (or `pipx install uv==0.12.13`). Node.js 24+ is required for the Pi/OMP tests.

Python dependencies are updated in `pyproject.toml`/`uv.lock`. After an accepted lock update, regenerate `server/requirements.txt` with `uv export --locked --no-dev --no-emit-project -o server/requirements.txt`. Never independently bump packages in this generated export; CI checks that it matches the lock.
