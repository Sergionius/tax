...
Если локальный HTTP endpoint временно недоступен, extension сохраняет задачу в `~/.local/state/tax/tasks.jsonl`. Backend остаётся источником истины: агент раз в несколько секунд забирает все недоставленные ответы и поэтому восстанавливает работу после сна, перезапуска процесса или reboot без ограничения срока ожидания. После успешной передачи ответ получает статус `delivered`; закрытая agterm-сессия приводит к терминальному `delivery_failed` без бесконечных повторов. Локальные финальные записи очищаются через 7 дней, история backend/iOS автоматически не удаляется.
