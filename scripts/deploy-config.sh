...
tax_deploy_config_load() {
    ...
    # An explicitly requested missing file is never silently ignored, even
    # when every required value happens to be present in the environment.
    if [[ -n "${TAX_DEPLOY_CONFIG:-}" && ! -r "$TAX_DEPLOY_CONFIG" ]]; then
        tax_deploy_die "TAX_DEPLOY_CONFIG is not a readable file"
        return 1
    fi
