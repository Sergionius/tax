...
"$UV_BIN" run --locked --extra dev ruff check server src tests scripts/deployment_checks.py
...
if command -v xcodebuild >/dev/null && [[ -f ios/tax/tax.xcodeproj/project.pbxproj ]]; then
  ...
  # Preserve any previous results even when an output parent is supplied.
  IOS_RESULT_PARENT="${IOS_RESULT_DIR:-$PREFLIGHT_TMP/ios-results}"
  mkdir -p "$IOS_RESULT_PARENT"
  IOS_RESULT_DIR="$(mktemp -d "$IOS_RESULT_PARENT/run.XXXXXXXX")"
