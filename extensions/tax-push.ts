...
function messageFailure(message: AnyMessage, agentDisplayName: string): string | undefined {
  ...
  if (message.stopReason === "error") return `${agentDisplayName} stopped with an unspecified provider error`;
  return undefined;
}
...
function makeNotificationBody(text: string, emptyBody: string, maxLength = 180): string {
  const oneLine = text.replace(/\s+/g, " ").trim();
  if (!oneLine) return emptyBody;
...
export function analyzeTurn(
  ...
  const agentDisplayName = agentName === "pi" ? "Pi" : "OMP";
  let userIndex = -1;
  ...
  for (const item of assistants) {
    const error = messageFailure(item.message, agentDisplayName);
  ...
  if (successful) {
    ...
    return {
      ...
      body: makeNotificationBody(logs, `${agentName} completed`),
      logs,
      outcome: "completed",
  ...
  if (failures.length > 0) {
    ...
    return {
      ...
      body: makeNotificationBody(
        `${classification.category} error after ${attempts} attempt${attempts === 1 ? "" : "s"}: ${latest}`,
        `${agentName} failed`,
      ),
  ...
  return {
    ...
    body: `${agentDisplayName} stopped without a final response`,
    logs: `${agentDisplayName} settled without assistant output or a reported error.`,
    outcome: "stopped",
