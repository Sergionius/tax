...
- **The backend processes notification content.** Completion pushes are not end-to-end encrypted: `tax run`, the Pi/OMP extensions and the Claude Code/Codex hooks send `title`, `body`, `context` and `logs` to the backend over HTTPS, where they are processed and stored (subject to the storage policy below).
...
Deleting rows (manually or through retention) removes them from the SQLite logical content, but it is **not a guaranteed physical erase from the storage medium**. Freelist pages, write-ahead logs, filesystem journaling, snapshots, and backups can retain data until the space is reused and eventually overwritten. Restoring a backup or recreating the database does not guarantee physical erasure either. Use encrypted storage from the outset and an appropriate platform-specific media sanitization or cryptographic-erasure procedure when physical erasure is required.
