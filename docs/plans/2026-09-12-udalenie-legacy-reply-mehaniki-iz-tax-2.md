...
- [x] Добавить README section `Upgrading from versions before 0.4.0`: installer снимает только `tax.agent`, remote-host не затрагивается, старые SQLite-колонки и локальные `agent.db`, logs, `tasks.jsonl` сохраняются и не используются.
- [x] Документировать generic `tax run pytest -q`, streaming, executable-based title, metadata, возврат exit code, отсутствие reply ожидания и регистрацию device token на backend.
- [x] В operations описать `push_status` как единственный актуальный статус доставки, `unknown` в CLI для исторических записей и редактированную публичную history-проекцию. Удалить описание активных legacy status transitions.
- [x] В protocol/compatibility docs называть runtime host `tax remote-host`, а не reply-agent; сохранить описание encryption, terminal input acknowledgement и запрета replay.
- [x] Удалить пять перечисленных полностью устаревших планов.
- [x] В трёх смешанных top-level планах сохранить полезную историю push filtering, device preferences и quality automation, удалив инструкции и acceptance criteria для reply architecture, detach, polling и удаляемых файлов. Обновить help smoke-команды.
- [x] Сохранить исходный removal document как текущий removal plan; убрать оставшиеся ссылки на удалённые документы из изменяемых смешанных планов.
...
- Decision: README migration-заметка размещена перед «Проверки разработки», а раздел `tax run` — после «Push deep links»; Alternatives: другой порядок секций; Reason: миграционная заметка относится к обновлению пакета, а `tax run` логически продолжает раздел про deep links; Side effects: none.
- Decision: в README migration-заметке упомянуты `agent.db` и `tasks.jsonl` как сохраняемые и неиспользуемые данные; Alternatives: не называть файлы; Reason: чекбокс прямо требует перечислить эти пути, а scan разрешает совпадения в migration note; Side effects: scan имеет одно разрешённое совпадение в README.
- Decision: в OPERATIONS.md добавлен абзац о публичной history-проекции и отсутствии изменений при GET, а предложение про legacy `pending`/`expired` transitions удалено; Alternatives: сохранить историческую справку о transitions; Reason: чекбокс требует удалить описание активных legacy status transitions и описать проекцию; Side effects: none.
- Decision: в remote-protocol/compatibility docs заменены только прозаические имена `tax-agent`/`tax-agent` на `tax remote-host`; Alternatives: переименовать wire identifier; Reason: wire identifier `tax-agent` в adapter/bridge сохранён по более раннему решению, меняется только название host-процесса в документации; Side effects: none.
